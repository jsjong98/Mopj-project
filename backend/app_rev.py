...
def get_file_cache_dirs(file_path=None):
    ...
    try:
        ...
        # 🔄 레거시 캐시 마이그레이션 확인 (Excel 파일 읽기 문제로 인한 해시 차이 해결)
        migrate_legacy_cache_if_needed(file_path)
...
def get_data_content_hash(file_path):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Data loading failed for {os.path.basename(file_path)}: {str(data_error)}")
            logger.warning(f"⚠️ Falling back to file content hash method")
            data_load_success = False
        
        if data_load_success and df is not None:
            # 데이터 로딩 성공: 전처리된 데이터로 해시 계산
            ...
            logger.info(f"✅ Data-based hash calculated: {file_hash} for {os.path.basename(file_path)}")
        else:
            # 데이터 로딩 실패: 파일 내용 해시 사용
            logger.info(f"🔄 Using file content hash for {os.path.basename(file_path)}")
            ...
            if file_content_hash:
                ...
                logger.error(f"❌ Both data and file hash calculation failed for {os.path.basename(file_path)}")
                return None
...
def load_existing_predictions_for_extension(file_path, target_date, compatibility_info):
    ...
    try:
        ...
        # CSV 파일들을 찾아서 로드
        csv_files = list(predictions_dir.glob("prediction_start_*.csv"))
        ...
        for csv_file in csv_files:
            try:
                ...
                # 날짜 필터링 (목표 날짜보다 이전 또는 같은 날짜만)
                if 'Date' in df.columns:
                    valid_predictions = df[df['Date'] <= target_date]
                    logger.info(f"📊 [EXTENSION_CACHE] Valid predictions after date filter: {len(valid_predictions)}")
...
def load_csv_safe_with_fallback(csv_path):
    """
    안전한 CSV 로드 함수 - xlwings 실패 시 pandas로 fallback
    ...
    try:
        ...
        if XLWINGS_AVAILABLE:
            logger.info(f"🔓 [SAFE_CSV] Attempting to load CSV with xlwings: {os.path.basename(csv_path)}")
            return load_csv_with_xlwings(csv_path)
        else:
            # xlwings가 없으면 pandas 사용
            logger.info(f"📊 [SAFE_CSV] Using pandas for CSV loading: {os.path.basename(csv_path)}")
            return pd.read_csv(csv_path)
    except Exception as xlwings_error:
        # xlwings 실패 시 pandas로 fallback
        logger.warning(f"⚠️ [SAFE_CSV] xlwings failed, falling back to pandas: {xlwings_error}")
        try:
            df = pd.read_csv(csv_path)
            logger.info(f"✅ [SAFE_CSV] Successfully loaded CSV with pandas fallback: {df.shape}")
            return df
        except Exception as pandas_error:
            logger.error(f"❌ [SAFE_CSV] Both xlwings and pandas failed: pandas error: {pandas_error}")
            raise xlwings_error  # 원래 오류를 다시 발생
...
def load_data(file_path, model_type=None, use_cache=True):
    ...
    if file_path.endswith('.csv'):
        ...
        df = load_csv_safe_with_fallback(file_path)
            ...
            try:
                ...
                if is_extension:
                    ...
                    if os.path.exists(existing_predictions_dir):
                        ...
                        # 파일명 패턴 시도
                        csv_patterns = [
                            f"prediction_start_{date_str}.csv",
                            f"prediction_{date_str}.csv"
                        ]
                        meta_patterns = [
...
def save_prediction_simple(prediction_results: dict, prediction_date):
    ...
    try:
        ...
        # ✅ 파일 경로 설정 (파일별 디렉토리 내) - 기존 파일 보존
        base_name = f"prediction_start_{first_date:%Y%m%d}"
        json_path = file_predictions_dir / f"{base_name}.json"
        csv_path = file_predictions_dir / f"{base_name}.csv"
        meta_path = file_predictions_dir / f"{base_name}_meta.json"
        attention_path = file_predictions_dir / f"{base_name}_attention.json"
        
        # 🔧 기존 파일이 있는 경우 타임스탬프 추가하여 보존
        if csv_path.exists() and meta_path.exists():
            logger.info(f"🔄 [PREDICTION_SAVE] Existing prediction found, preserving with timestamp")
            timestamp = datetime.now().strftime('%H%M%S')
            json_path = file_predictions_dir / f"{base_name}_{timestamp}.json"
            csv_path = file_predictions_dir / f"{base_name}_{timestamp}.csv"
            meta_path = file_predictions_dir / f"{base_name}_{timestamp}_meta.json"
            attention_path = file_predictions_dir / f"{base_name}_{timestamp}_attention.json"
            logger.info(f"  📁 New files will be saved with timestamp: {timestamp}")
        
        # 🔧 MA 파일 경로도 같은 방식으로 처리
        ma_path = file_predictions_dir / f"{base_name}_ma.json"
        if csv_path.name != f"{base_name}.csv":
            # 타임스탬프가 추가된 경우 MA 파일에도 동일한 타임스탬프 적용
            timestamp = csv_path.stem.split('_')[-1]
            ma_path = file_predictions_dir / f"{base_name}_{timestamp}_ma.json"
        ...
        # ✅ Attention 데이터 저장 (있는 경우) - NaN 값 안전 처리
        ...
        # ✅ 인덱스 업데이트 - 안전하게 직렬화된 메타데이터 사용
        update_predictions_index_simple(safe_meta)
            ...
            for existing_file in existing_files:
                try:
                    ...
                    if existing_predictions_dir.exists():
                        ...
                        exact_csv_other = existing_predictions_dir / f"prediction_start_{date_str}.csv"
...
class VARMAXSemiMonthlyForecaster:
    ...
    def generate_predictions_varmax(self, current_date, var_num):
        ...
        try:
            ...
            # 예측 결과를 표준 형식으로 변환 - JSON 안전성 보장
            predictions = []
            for date, value in self.final_forecast_var.iterrows():
                # 🔧 안전한 값 처리
                prediction_value = safe_serialize_value(value[self.result_var])
                predictions.append({
                    'Date': format_date(date),
                    'Prediction': prediction_value,
                    'Actual': None  # 실제값은 미래이므로 None (JSON 안전)
                })
