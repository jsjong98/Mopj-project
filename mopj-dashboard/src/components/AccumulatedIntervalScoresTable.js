// 새 파일: AccumulatedIntervalScoresTable.js
...
const AccumulatedIntervalScoresTable = ({ data }) => {
  if (!data || !data.accumulated_interval_scores || data.accumulated_interval_scores.length === 0) {
    return (
      <div style={styles.noData}>
        <p style={styles.noDataText}>신뢰 날짜 구매 의사결정 구간 데이터가 없습니다</p>
      </div>
    );
  }

  // 상위 10개 구간만 표시
  const displayData = data.accumulated_interval_scores.slice(0, 10);
        ...
        <table style={styles.table}>
          <thead style={styles.thead}>
            <tr>
              ...
              <th style={styles.th}>누적 점수</th>
              <th style={styles.th}>출현 횟수</th>
            </tr>
          </thead>
          <tbody style={styles.tbody}>
            {displayData.map((interval, idx) => (
              <tr key={idx} style={styles.tr(idx === 0)}>
                ...
                <td style={styles.td(false)}>{interval.count || 1}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {bestInterval && (
        <div style={styles.recommendationBox}>
          ...
          <div style={styles.recommendationDetail}>
            ...
            <p><span style={styles.boldText}>누적 점수:</span> {bestInterval.score} (최고점)</p>
            <p><span style={styles.boldText}>출현 횟수:</span> {bestInterval.count || 1}회</p>
          </div>
        </div>
      )}
    </div>
...
export default AccumulatedIntervalScoresTable;
